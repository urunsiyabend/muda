...
impl<T: Tweenable + Clone> Tween<T> {
    ...
    /// Does not produce a visual jump — animation continues from wherever it is.
    ...
    /// Get the current value (owned clone).
...
/// Trait for values that can be tweened (linearly interpolated).
pub trait Tweenable: Clone + PartialEq {
    /// Linearly interpolate between self and other at factor `t` (0.0..=1.0).
    fn lerp(&self, other: &Self, t: f32) -> Self;
}

// ---------- Primitive Tweenable impls ----------
...
mod tests {
    ...
    fn test_f64_lerp() {
        let a = 0.0_f64;
        assert!((a.lerp(&1.0, 0.5) - 0.5).abs() < 0.0001);
    }

    #[test]
    fn test_i32_lerp() {
        let a = 0_i32;
        assert_eq!(a.lerp(&100, 0.5), 50);
    }

    #[test]
    fn test_u8_lerp() {
        let a = 0_u8;
        assert_eq!(a.lerp(&200, 0.5), 100);
    }

    #[test]
    fn test_tuple_lerp() {
        let a = (0.0_f32, 0.0_f32);
        let mid = a.lerp(&(10.0, 20.0), 0.5);
    ...
    fn test_array4_lerp() {
        let a = [0.0_f32; 4];
        let mid = a.lerp(&[4.0, 8.0, 12.0, 16.0], 0.5);
        assert!((mid[0] - 2.0).abs() < 0.001);
        assert!((mid[3] - 8.0).abs() < 0.001);
    }
    ...
    #[test]
    fn test_tween_retarget_no_jump() {
        // retarget() must continue from current position, not jump
        let mut tween = Tween::new(0.0_f32, 100.0, Duration::from_millis(200), Easing::Linear);
        tween.start();
        // Simulate a tiny bit of time passing by manually setting values
        // We can test the retarget logic structurally:
        tween.retarget(50.0);
        // After retarget the start must equal the current value (not the original start)
    ...
    fn test_tween_finish() {
        let mut tween = Tween::ease_out(0.0_f32, 100.0, Duration::from_millis(300));
        tween.start();
        tween.finish();
        assert_eq!(*tween.value(), 100.0);
        assert!(tween.is_complete());
    ...
    #[test]
    fn test_tween_progress_idle() {
        let tween = Tween::ease_out(0.0_f32, 100.0, Duration::from_millis(300));
        assert_eq!(tween.progress(), 0.0);
    }

    #[test]
    fn test_tween_progress_complete() {
        let mut tween = Tween::ease_out(0.0_f32, 100.0, Duration::from_millis(300));
        tween.finish();
    ...
    #[test]
    fn test_tween_color_interpolation() {
        let start = Color::black();
        let end = Color::white();
        let mut tween = Tween::new(start, end, Duration::from_millis(300), Easing::EaseOut);
        tween.start();
        tween.finish();
        let v = tween.value();
        assert!((v.r - 1.0).abs() < 0.001);
        assert!((v.g - 1.0).abs() < 0.001);
        assert!((v.b - 1.0).abs() < 0.001);
    }
}
